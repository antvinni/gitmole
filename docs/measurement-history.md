...
| findings per repository, median and p90 | development | 23.5 and 28 |
| findings the default report spells out that are labelled actionable | development and well-kept | 78% of 77, 82% labelled |
...
| wall time and peak memory | development | 766 s, 1130 MB |
...
49 numeric thresholds across the rules, each moved 10, 25 and 50% either side on the development set: 29 flat, 11 fragile, 1 moderate, 6 silent, 2 untested. None of them records where its value came from, so each counts as fitted until a line beside it says otherwise.
...
| import_cycles | min_resolved | 0.6 | flat | 3 / 3 / 3 |
