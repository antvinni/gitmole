...
| `--install-tools` | Download the five tools at the versions gitmole pins, from the release archives the Homebrew formula installs and checked against the same sha256, into gitmole's own directory: `GITMOLE_TOOLS` if set, else `~/Library/Application Support/gitmole/tools` on macOS or `$XDG_DATA_HOME/gitmole/tools` (default `~/.local/share/gitmole/tools`) elsewhere. A run looks there before PATH. Takes no target; other options are ignored. Exit 0 when all five landed, 1 when one did not (no build for this platform, a hash mismatch, no network) with the reason named. Apart from cloning a remote target you name, gitmole reaches the network only for `--install-tools` or a yes to the missing-tools question; a scan of a local clone never does. |
