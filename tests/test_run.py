...
class ChangedFiles(unittest.TestCase):
    def test_lists_paths_changed_since_the_merge_base(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with open(os.path.join(d, "b.py"), "a") as fh: fh.write("y\n")
            os.makedirs(os.path.join(d, "dir"))
            with open(os.path.join(d, "dir", "c.py"), "w") as fh: fh.write("z\n")
            git("add", "-A"); git("commit", "-q", "-m", "work")
            git("switch", "-q", "main")
            with open(os.path.join(d, "a.py"), "a") as fh: fh.write("main moved on\n")
