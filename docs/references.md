...
- Kolassa, Riehle and Salim, "A Model of the Commit Size Distribution of Open Source," *SOFSEM* 2013. [arXiv:1408.4974](https://arxiv.org/pdf/1408.4974): the repository's own 99th percentile in `maat.sweeping` and `maat.oversized`, and the `sweeping_commits` finding.
...
- Maldonado and Shihab, "Detecting and quantifying different types of self-admitted technical debt," *MTD* 2015. [PDF](http://users.encs.concordia.ca/~eshihab/pubs/Maldonado_MTD2015.pdf): the TODO, FIXME, XXX and HACK markers in `structure.py` and the `debt_in_hotspots` finding. Their five debt types (design, defect, documentation, requirement, test) are detected by Potdar and Shihab's list of 62 comment patterns, a word list learned from a handful of projects; gitmole keys rules on shapes and declarations only, so the five types are deliberately not detected.
...
- Oyetoyan, Falleri, Dietrich and Jezek, "Circular dependencies and change-proneness: An empirical study," *SANER* 2015. [DOI](https://doi.org/10.1109/SANER.2015.7081834): the `import_cycles` finding. The study is of Java classes and gitmole applies it to Python, JavaScript and TypeScript files; it found that neither subtyping nor where a cycle sits in the package tree tells a harmful cycle from a harmless one, which is why the finding is `info` and gives no verdict.
...
- Rosa et al., "Evaluating SZZ Implementations Through a Developer-informed Oracle," *ICSE* 2021, extended *JSS* 2023. [arXiv:2102.03300](https://arxiv.org/pdf/2102.03300): R-SZZ in `szz.py` and `evaluate --szz`. Rosa measured R-SZZ at 66% precision and 73% with issue-date filtering, which discards candidates committed after the bug was reported; gitmole runs offline over a clone and git carries no issue dates, so the 73% variant is not implementable here and `szz.py` is the 66% one.
...
- Zimmermann and Nagappan, "Predicting Defects using Network Analysis on Dependency Graphs," *ICSE* 2008. [ACM](https://dl.acm.org/doi/10.1145/1368088.1368161): the `dependents` count on `--risk` and the `--hook` summary, what imports a touched file directly and through other files. Shown as context, not scored: the paper's network measures predicted defects on Windows Server 2003 binaries, and nothing here has measured them on gitmole's corpus.
